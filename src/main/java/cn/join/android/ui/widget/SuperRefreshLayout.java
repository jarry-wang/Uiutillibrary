...
public class SuperRefreshLayout extends SwipeRefreshLayout {
    ...
    private Adapter<RecyclerView.ViewHolder> adapter;
    ...
    /**
     * 加载完毕
     * @param ifOver 是否还有下一页
     */
    public void loadComplete(boolean ifOver){
        if(adapter == null){
            throw new RuntimeException("must call method setAdapter to bind data");
        }
        adapter.setState(ifOver ? Adapter.STATE_END : Adapter.STATE_MORE);
    }
